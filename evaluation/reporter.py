...
class ReportGenerator:
    ...
    def generate_report(
        ...
        Args:
            ...
            plot_dir: Directory with plot images (auto-generated if None)
        """
    ...
    def _generate_plots(self, plot_dir: str) -> Dict[str, str]:
        ...
        try:
            performance_path = str(Path(plot_dir) / 'performance.png')
            viz.plot_performance_comparison(save_path=performance_path)
            plot_paths['performance'] = performance_path
        except Exception as e:
            logger.warning(f"Failed to generate performance plot: {e}")
        
        try:
            retrieval_path = str(Path(plot_dir) / 'retrieval.png')
            viz.plot_retrieval_comparison(save_path=retrieval_path)
            plot_paths['retrieval'] = retrieval_path
        except Exception as e:
            logger.warning(f"Failed to generate retrieval plot: {e}")
        
        try:
            radar_path = str(Path(plot_dir) / 'radar.png')
            viz.plot_radar_chart(
                metrics=['latency_ms_per_token', 'throughput_tokens_per_sec', 
                        'average_accuracy', 'exact_match', 'f1_score'],
...
'''
    ...
    def _generate_plots_section(self, plot_paths: Dict[str, str]) -> str:
        ...
        plot_titles = {
            ...
            'retrieval': 'Retrieval/RAG Comparison',
    ...
    def _generate_detailed_results(self) -> str:
        ...
        sections = [
            ...
            ('Performance', ['perplexity', 'average_accuracy']),
            ('Retrieval', ['precision_at_3', 'exact_match', 'f1_score', 'faithfulness'])
        ]
    ...
    def _create_comparison_content(self, baseline_idx: int, comp_idx: int) -> str:
        ...
        for metric in metrics:
            ...
            is_improvement = diff > 0 if 'accuracy' in metric or 'throughput' in metric else diff < 0
    ...
    def _generate_metadata_section(self) -> str:
        ...
        html = ['<div class="section">', '<h2>ℹ️ Metadata</h2>']
        
        for name, result in zip(self.result_names, self.results):
            metadata = result.get('metadata', {})
            if metadata:
                html.append(f'<h3>{self._escape_html(name)}</h3>')
                html.append('<ul>')
                for key, value in metadata.items():
                    html.append(f'<li><strong>{key}:</strong> {self._escape_html(str(value))}</li>')
                html.append('</ul>')
